...
while (my $in=<>)
{
    chop $in; # Chop off the last char ##@DavidSiegl: use trim function in python?

    $in=~s/[\,!\?"]//g; # remove punctuation ##@DavidSiegl: regex
    # $in=~s/\./ /g; # remove . ?
    $in=~s/\s[^\s]*\*[^\s]*/ €/g; # convert word with a *-wildcard to € ##@DavidSiegl: regex sub
    $in=~s/\s+$//;
